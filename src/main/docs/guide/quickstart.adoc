...
[.lang-python]
NOTE: picocli reads the `@Command`, `@Option` and `@Parameters` annotations reflectively from the Java class of the command, so the Python compiler must be told to keep them on the generated class: add `-Amicronaut.introspection.allowReflection=<package>.*` to the compiler arguments (`micronautBuild { python { compilerArgs.add(...) } }` in Gradle) or annotate the command with `@AllowsReflection` (`micronaut.core.annotation`). A command with `@Option` / `@Parameters` attributes is also `@Introspected`: the attribute annotations are copied onto the fields of the generated class only for an introspected class.
